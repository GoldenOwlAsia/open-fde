...
test("--only runs a single check", () => {
  const result = runDefaultChecks(emptyInventory, null, { only: ["pii-policy-undefined"] });
  assert.deepEqual(result.findings.map((f) => f.id), ["pii-policy-undefined"]);
});

test("--skip removes a check", () => {
  const result = runDefaultChecks(emptyInventory, null, { skip: ["pii-policy-undefined"] });
  assert.equal(result.findings.find((f) => f.id === "pii-policy-undefined"), undefined);
  assert.ok(result.findings.length > 0);
});

test("unknown check ids in only/skip fail with the available ids", () => {
  assert.throws(
...
test("severity override with a reason downgrades a finding and records the reason as evidence", () => {
...
test("an override without a reason is rejected", () => {
  ...
  assert.throws(() => runDefaultChecks(emptyInventory, engagement), /missing a reason/);
});

test("an override for an unknown check id is rejected", () => {
  const engagement: Engagement = {
    spec: { checks: { overrides: [{ id: "nope", severity: "info", reason: "typo" }] } }
  };
  assert.throws(() => runDefaultChecks(emptyInventory, engagement), /unknown check id "nope"/);
});

test("every builtin check has a unique id and a description", () => {
