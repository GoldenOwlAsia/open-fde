...
test("regions outside the declared residency boundary are critical", async () => {
...
test("residency check stays silent when all regions are allowed or none are declared", async () => {
  const inventory = inventoryWith({ regions: [{ region: "ap-southeast-1", evidence: "main.tf:3" }] });
  const allowed: Engagement = {
    spec: { constraints: { dataResidency: { allowedRegions: ["ap-southeast-1"] } } }
  };
  assert.equal(
    (await runDefaultChecks(inventory, allowed)).findings.find((f) => f.id === "data-residency-violation"),
    undefined
  );
  // No allowedRegions declared → the check does not apply at all.
  assert.equal(
    (await runDefaultChecks(inventory, null)).findings.find((f) => f.id === "data-residency-violation"),
    undefined
  );
});

test("secret suspects produce a critical finding with file:line but never a value", async () => {
  const inventory = inventoryWith({
    secretSuspects: [
...
test("write signals against a declared read_only system are critical", async () => {
  const inventory = inventoryWith({
...
test("write signals against a read_write system are fine", async () => {
  const inventory = inventoryWith({
    writeSignals: [{ systemType: "postgres", file: "src/db.ts", line: 40, pattern: "SQL INSERT/UPDATE/DELETE" }]
  });
  const engagement: Engagement = {
...
test("success metrics with name and target satisfy the metrics check", async () => {
  const engagement: Engagement = {
    spec: { successMetrics: [{ name: "resolution_rate", target: ">= 0.70" }] }
  };
  const result = await runDefaultChecks(inventoryWith({}), engagement);
  ...
  const finding = (await runDefaultChecks(inventoryWith({}), incomplete)).findings.find(
